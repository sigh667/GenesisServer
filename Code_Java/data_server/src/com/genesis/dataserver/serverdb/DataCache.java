...
public class DataCache {
    ...
    private HashMap<IRedisKey<? extends Serializable, ? extends EntityWithRedisKey<?>>, DbOp> map =
            new HashMap<>();
    /**记录最老的那个缓存数据进入的时间点*/
    ...
    public void put(DirtyDataInfo dirtyDataInfo) {
        ...
        IRedisKey<? extends Serializable, ? extends EntityWithRedisKey<?>> redisKey =
                dirtyDataInfo.getRedisKey();
        DbOp operateType = dirtyDataInfo.getDbOp();
        if (redisKey == null) {
            logger.warn(
                    "DataChche:put(DirtyDataInfo dirtyDataInfo) dirtyDataInfo.getRedisKey() is null!");
            return;
        }
        if (operateType == null) {
            logger.warn(
                    "DataChche:put(DirtyDataInfo dirtyDataInfo) dirtyDataInfo.getDbOp() is null!");
            return;
        }
    ...
    private void flushModifiedEntity(HibernateDBService dbservice) {
        ...
        IRedisResponse<List<IRedisResponse<?>>> results = pip.exec(new PipelineProcess() {
            ...
            @Override
            public void apply() {
                for (Entry<IRedisKey<? extends Serializable, ? extends EntityWithRedisKey<?>>, DbOp> entry : map
                        .entrySet()) {
                    IRedisKey<? extends Serializable, ? extends EntityWithRedisKey<?>> key =
                            entry.getKey();
                    ...
                    try {
                        ...
                        logger.error(
                                "DataCache:flushModifiedEntity() key.getEntityRedisOp().getEntityFromRedis(this, key) throw Exception!",
                                e);
                        continue;
                    }
        ...
        for (int i = 0; i < resultList.size(); i++) {
            @SuppressWarnings("unchecked") IRedisResponse<BaseEntity> iRedisResponse =
                    (IRedisResponse<BaseEntity>) resultList.get(i);
            ...
            if (tempEntity == null) {
                ...
                logger.warn("DataCache:flushModifiedEntity() entity[{}] to save or update is null!",
                        dirtyDataInfo.getRedisKey().getEntityType().getName());
                continue;
            }
        ...
        try {
            ...
            logger.error(
                    "DataCache:flushModifiedEntity() dbservice.saveOrUpdateBatch(saveOrUpdateList) throw Exception!",
                    e);
            ...
            for (BaseEntity entity : saveOrUpdateList) {
                try {
                    ...
                    logger.error(
                            "DataCache:flushModifiedEntity() dbservice.saveOrUpdate(entity) throw Exception!",
                            e1);
                }
            }
    ...
    private void flushDeleteEntity(HibernateDBService dbservice) {
        ...
        for (Iterator<Entry<IRedisKey<? extends Serializable, ? extends EntityWithRedisKey<?>>, DbOp>>
                iterator = map.entrySet().iterator(); iterator.hasNext(); ) {
            Entry<IRedisKey<? extends Serializable, ? extends EntityWithRedisKey<?>>, DbOp> entry =
                    iterator.next();
