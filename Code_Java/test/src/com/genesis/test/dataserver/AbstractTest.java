...
public class AbstractTest {
    ...
    public void config() throws Exception {
        ...
        //初始化配置文件
        mappingConf =
                ConfigBuilder.buildConfigFromFileName("dataserver/MappingTest.conf", Mapping.class);

        //初始化数据库
        ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
        URL url = classLoader.getResource("h2_hibernate_test.cfg.xml");
        String[] dbResources = new String[]{"hibernate_query.xml"};
        dbService = new HibernateDBService("com.mokylin.bleach.gamedb.orm.entity", new Properties(),
                url, dbResources);
        ...
        DataServerConfig dsc =
                new DataServerConfig(oldConfig.serverConfig, oldConfig.sqlProperties, mappingConf);
