package com.genesis.test.gameserver.shop;
...
import com.genesis.gameserver.shop.shop.Good;
...
public class ShopTest {
    ...
    public void prepare_for_each_test_case() {
        ...
        // 模拟商店相关的模板缓存数据
        ShopRelatedTemplateBufferData shopRelatedTmplBufferData =
                mock(ShopRelatedTemplateBufferData.class);
        when(shopRelatedTmplBufferData.getShopGoodTmplList(Mockito.any(ShopType.class)))
                .thenReturn(tmpls);

        // 物品相关的模板缓存数据
        ItemRelatedTemplateBufferData itemRelatedTmplBufferData =
                mock(ItemRelatedTemplateBufferData.class);
        ...
        when(templateService.get(1, ShopItemStoreRoomTemplate.class))
                .thenReturn(createStoreRoomTemplate());
        ...
        ShopDiscount shopDiscount = new ShopDiscount(null, 1, 1, ShopType.GENERAL, 5000, 1, 1000,
                System.currentTimeMillis() + TimeUtils.DAY);
    ...
    public void the_item_template_id_should_be_correct() {
        ...
        try {
            itemTmpls =
                    (Table<ItemType, Integer, List<ItemTemplate>>) itemTmplsField.get(bufferData);
        ...
        ItemTemplate itemTmpl = bufferData.randomSelectItemTmpl(
                Lists.newArrayList(ItemType.Equipment, ItemType.EquipmentMaterial), 5, 5);
