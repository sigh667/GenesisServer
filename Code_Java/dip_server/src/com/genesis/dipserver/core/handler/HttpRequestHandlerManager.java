...
public class HttpRequestHandlerManager {
    ...
    static {
        Set<Class<?>> handlerClasses = PackageUtil
                .getPackageClasses("com.mokylin.bleach.dipserver", new Predicate<Class<?>>() {
                    @Override
        ...
        for (Class<?> clazz : handlerClasses) {
            ...
            if (handlers.containsKey(path)) {
                throw new RuntimeException(
                        String.format("register multitimes with same path[%s]", path));
            }
