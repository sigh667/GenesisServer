...
public class AgentServer {
    ...
    public static void main(String[] args) {
        ...
        ActorRef actorLocal = Globals.getAkka().getActorSystem().actorOf(
                Props.create(ISCActorSupervisor.class, config.serverConfig, Globals.getISCService(),
                        "com.mokylin.bleach.agentserver", new GameServerRemoteFactory()),
        ...
        // 测试发消息
        Optional<IRemote> remote =
                Globals.getISCService().getRemote(loginConfig.serverType, loginConfig.serverId);
    ...
    private static boolean connectToLoginServer() {
        ...
        if (!remoteActorManager.connectRemote(
                new RemoteServerConfig(loginConfig.serverType, loginConfig.serverId,
                        loginConfig.akkaConfig.ip, loginConfig.akkaConfig.port),
                ISCActorSupervisor.ACTOR_NAME)) {
            log.info("Connect to LoginServer failed! id: [{}], ip: [{}], port: [{}]",
                    loginConfig.serverId, loginConfig.akkaConfig.ip, loginConfig.akkaConfig.port);
            return false;
        }
    ...
    private static boolean registerToRemote(ServerType serverType, int sId, ActorRef actorRef) {
        ...
        try {
            Future<Object> future =
                    Patterns.ask(actorRef, new RegisterToRemote(serverType, sId), timeout);
            RegisterToRemoteResult result =
                    (RegisterToRemoteResult) Await.result(future, timeout.duration());
            if (result == null || !result.isSuccess) {
                throw new RuntimeException("AgentServer register to LoginServer Failed!");
            }
            return true;
        } catch (Exception e) {
