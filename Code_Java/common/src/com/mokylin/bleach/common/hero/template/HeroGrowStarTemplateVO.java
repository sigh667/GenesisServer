...
public abstract class HeroGrowStarTemplateVO extends TemplateObject {
    ...
    public void setStarUpCostMoney(long starUpCostMoney) {
        if (starUpCostMoney == 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[升到下一星级需要的金币]starUpCostMoney不可以为0");
        }
    ...
    public String toString() {
        return "HeroGrowStarTemplateVO[growthRate=" + growthRate + ",starUpCostfragment=" +
                starUpCostfragment + ",starUpCostMoney=" + starUpCostMoney + ",hireCost=" +
                hireCost + ",]";
