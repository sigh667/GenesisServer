...
public abstract class HeroGroupTemplateVO extends TemplateObject {
    ...
    public void setHireCostItemCount(int hireCostItemCount) {
        if (hireCostItemCount == 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[直接召唤所需灵石数量]hireCostItemCount不可以为0");
        }
        if (hireCostItemCount < 1) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[直接召唤所需灵石数量]hireCostItemCount的值不得小于1");
        }
    ...
    public void setDecomposeToItemCount(int decomposeToItemCount) {
        if (decomposeToItemCount == 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[整卡兑换碎片数]decomposeToItemCount不可以为0");
        }
        if (decomposeToItemCount < 1) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[整卡兑换碎片数]decomposeToItemCount的值不得小于1");
        }
    ...
    public String toString() {
        return "HeroGroupTemplateVO[type=" + type + ",itemId=" + itemId + ",baseStar=" + baseStar +
                ",hireCostItemCount=" + hireCostItemCount + ",decomposeToItemCount=" +
                decomposeToItemCount + ",isAlwaysInHeroPanel=" + isAlwaysInHeroPanel + ",]";
