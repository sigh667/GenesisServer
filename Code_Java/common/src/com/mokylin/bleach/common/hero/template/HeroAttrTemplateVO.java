...
public abstract class HeroAttrTemplateVO extends TemplateObject {
    ...
    public void setRoleAttrId(int roleAttrId) {
        if (roleAttrId == 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[对应的role_attr文件中的ID]roleAttrId不可以为0");
        }
    ...
    public String toString() {
        return "HeroAttrTemplateVO[heroGroupId=" + heroGroupId + ",name=" + name + ",desc=" + desc +
                ",icon=" + icon + ",nextHeroId=" + nextHeroId + ",qualityId=" + qualityId +
                ",qualityUpCost=" + qualityUpCost + ",raceId=" + raceId + ",normalSkillId=" +
                normalSkillId + ",specialSkillId=" + specialSkillId + ",secondarySkillIds=" +
                secondarySkillIds + ",equipIds=" + equipIds + ",roleAttrId=" + roleAttrId + ",]";
