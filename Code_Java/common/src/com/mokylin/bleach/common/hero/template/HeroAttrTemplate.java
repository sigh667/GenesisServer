...
public class HeroAttrTemplate extends HeroAttrTemplateVO {
    ...
    public void check() throws TemplateConfigException {
        ...
        if (hasNextQualityHero()) {
            //可以升品
            //那么检查下一品的英雄是否存在
            HeroAttrTemplate nextTemplate =
                    GlobalData.getTemplateService().get(nextHeroId, HeroAttrTemplate.class);
    ...
    public void patchUp() throws Exception {
        RoleAttrTemplate roletemplate =
                GlobalData.getTemplateService().get(this.getRoleAttrId(), RoleAttrTemplate.class);
        ...
        for (TempAttrNode3Col tempAttrNode : attributeList) {
            final List<BattlePropEffect> tempList =
