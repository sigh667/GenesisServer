...
public abstract class BuffTemplateVO extends TemplateObject {
    ...
    public void setWorkingTime(
            com.mokylin.bleach.common.combat.enumeration.BuffWorkingTime workingTime) {
        if (workingTime == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[WorkingTime(作用时间)]workingTime不可以为空");
        }
    ...
    public void setSpellBuffHitType(
            com.mokylin.bleach.common.combat.enumeration.SpellBuffHitType spellBuffHitType) {
        if (spellBuffHitType == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[Buff命中规则]spellBuffHitType不可以为空");
        }
    ...
    public String toString() {
        return "BuffTemplateVO[name=" + name + ",workingTime=" + workingTime + ",remainTime=" +
                remainTime + ",buffType=" + buffType + ",isCanStop=" + isCanStop +
                ",spellBuffHitType=" + spellBuffHitType + ",effectMain=" + effectMain +
                ",effectEnd=" + effectEnd + ",attributeList=" + attributeList + ",]";
