...
public abstract class SpellTemplateVO extends TemplateObject {
    ...
    public void setIcon(int icon) {
        if (icon == 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[Icon]icon不可以为0");
        }
        this.icon = icon;
    }
    ...
    public void setTargetSelector(
            com.mokylin.bleach.common.combat.enumeration.TargetSelectorType targetSelector) {
        if (targetSelector == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[目标选择方式]targetSelector不可以为空");
        }
    ...
    public void setRangeSelector(
            com.mokylin.bleach.common.combat.enumeration.RangeSelectorType rangeSelector) {
        if (rangeSelector == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[技能范围]rangeSelector不可以为空");
        }
    ...
    public void setYuanSuLeiXing(
            com.mokylin.bleach.common.combat.enumeration.YuanSuLeiXing yuanSuLeiXing) {
        if (yuanSuLeiXing == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[元素类型]yuanSuLeiXing不可以为空");
        }
    ...
    public void setSpellDamageType(
            com.mokylin.bleach.common.combat.enumeration.SpellDamageType spellDamageType) {
        if (spellDamageType == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[招式伤害类型]spellDamageType不可以为空");
        }
    ...
    public void setSpellExpType(
            com.mokylin.bleach.common.combat.enumeration.SpellExpType spellExpType) {
        if (spellExpType == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[招式结算公式类型]spellExpType不可以为空");
        }
    ...
    public void setISpellCorrectionParameter(int iSpellCorrectionParameter) {
        if (iSpellCorrectionParameter == 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[技能修正值(万分比)]iSpellCorrectionParameter不可以为0");
        }
    ...
    public void setAttributeList(
            List<com.mokylin.bleach.common.core.excelmodel.TempAttrNode2Col> attributeList) {
        if (attributeList == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[附带的属性]attributeList不可以为空");
        }
    ...
    public String toString() {
        return "SpellTemplateVO[icon=" + icon + ",name=" + name + ",desc=" + desc + ",priority=" +
                priority + ",mpCost=" + mpCost + ",cd=" + cd + ",targetSelector=" + targetSelector +
                ",rangeSelector=" + rangeSelector + ",yuanSuLeiXing=" + yuanSuLeiXing +
                ",spellType=" + spellType + ",isVisible=" + isVisible + ",spellDamageType=" +
                spellDamageType + ",spellExpType=" + spellExpType + ",levelRelatedParameter=" +
                levelRelatedParameter + ",iSpellCorrectionParameter=" + iSpellCorrectionParameter +
                ",IsBiZhong=" + IsBiZhong + ",IsBiBaoJi=" + IsBiBaoJi + ",IsNoBaoJi=" + IsNoBaoJi +
                ",IsWuFaGeDang=" + IsWuFaGeDang + ",IsBiPoJia=" + IsBiPoJia + ",AttackRange=" +
                AttackRange + ",strActionID=" + strActionID + ",attributeList=" + attributeList +
                ",]";
