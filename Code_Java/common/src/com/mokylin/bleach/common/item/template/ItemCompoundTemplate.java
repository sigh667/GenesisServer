        ...
        for (ItemCompoundMaterial eachMeterial : compoundMaterials) {
            if (eachMeterial.getMaterialTemplateId() > 0) {
                ...
                if (!GlobalData.getTemplateService()
                        .isTemplateExist(eachMeterial.getMaterialTemplateId(),
                                ItemTemplate.class)) {
                    throw new TemplateConfigException(this.getSheetName(), this.getId(),
                            "物品合成的原材料【" + eachMeterial.getMaterialTemplateId() + "】在Item表中不存在！");
                }
                if (eachMeterial.getAmount() <= 0) {
                    throw new TemplateConfigException(this.getSheetName(), this.getId(),
                            "物品合成的原材料【" + eachMeterial.getMaterialTemplateId() + "】所需的数量必须大于0！");
                }
                if (duplicatedMeterial.containsKey(eachMeterial.getMaterialTemplateId())) {
                    ...
                    throw new TemplateConfigException(this.getSheetName(), this.getId(),
                            "物品合成的原材料【" + eachMeterial.getMaterialTemplateId() + "】重复！");
                }
