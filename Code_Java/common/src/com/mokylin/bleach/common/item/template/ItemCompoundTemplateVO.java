...
public abstract class ItemCompoundTemplateVO extends TemplateObject {
    ...
    public void setCompoundPrice(long compoundPrice) {
        if (compoundPrice == 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[合成价格]compoundPrice不可以为0");
        }
        if (compoundPrice < 1) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[合成价格]compoundPrice的值不得小于1");
        }
    ...
    public void setCompoundMaterials(
            com.mokylin.bleach.common.item.template.ItemCompoundMaterial[] compoundMaterials) {
        if (compoundMaterials == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[合成需要的物品模板ID]compoundMaterials不可以为空");
        }
    ...
    public String toString() {
        return "ItemCompoundTemplateVO[compoundPrice=" + compoundPrice + ",compoundMaterials=" +
                compoundMaterials + ",]";
