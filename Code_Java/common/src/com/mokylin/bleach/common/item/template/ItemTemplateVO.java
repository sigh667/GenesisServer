...
public abstract class ItemTemplateVO extends TemplateObject {
    ...
    public void setCurrencyPropId(
            com.mokylin.bleach.common.currency.CurrencyPropId currencyPropId) {
        if (currencyPropId == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[出售货币类型]currencyPropId不可以为空");
        }
    ...
    public void setAttributeList(
            List<com.mokylin.bleach.common.core.excelmodel.TempAttrNode3Col> attributeList) {
        if (attributeList == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[装备附加的属性]attributeList不可以为空");
        }
    ...
    public void setFromMissionTemplateIds(int[] fromMissionTemplateIds) {
        if (fromMissionTemplateIds == null) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[物品产生的关卡ID]fromMissionTemplateIds不可以为空");
        }
    ...
    public String toString() {
        return "ItemTemplateVO[name=" + name + ",desc=" + desc + ",iconId=" + iconId +
                ",itemType=" + itemType + ",qualityType=" + qualityType + ",currencyPropId=" +
                currencyPropId + ",sellPrice=" + sellPrice + ",isValid=" + isValid + ",level=" +
                level + ",useLevel=" + useLevel + ",maxOverlap=" + maxOverlap + ",func=" + func +
                ",funcParams=" + funcParams + ",attributeList=" + attributeList +
                ",fromMissionTemplateIds=" + fromMissionTemplateIds + ",fromOtherPlaceDesc=" +
                fromOtherPlaceDesc + ",]";
