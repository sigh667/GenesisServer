...
public abstract class ShopTemplateVO extends TemplateObject {
    ...
    public void setTempOpenDuration(int tempOpenDuration) {
        if (tempOpenDuration == 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(), 0,
                    "[临时开启时长（分钟）]tempOpenDuration不可以为0");
        }
    ...
    public String toString() {
        return "ShopTemplateVO[shopName=" + shopName + ",mainShopCurrency=" + mainShopCurrency +
                ",tempOpenDuration=" + tempOpenDuration + ",]";
