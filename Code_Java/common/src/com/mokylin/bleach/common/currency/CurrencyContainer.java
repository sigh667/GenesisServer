...
public class CurrencyContainer {
    ...
    /** 金币属性管理器 */
    protected EnumArray<CurrencyPropId, Long> currencyProp =
            EnumArray.create(CurrencyPropId.class, Long.class, 0L);
    ...
    public boolean isMoneyEnough(CurrencyPropId currencyPropId, long value) {
        if (currencyPropId == null || currencyPropId == CurrencyPropId.CHARGE_DIAMOND ||
                currencyPropId == CurrencyPropId.FREE_DIAMOND || value <= 0) {
            throw new IllegalArgumentException(
                    String.format("参数有误：value=%d (value必须大于0), currency=%s (currency不能为空，且不能是钻石)",
                            value, currencyPropId));
        }
