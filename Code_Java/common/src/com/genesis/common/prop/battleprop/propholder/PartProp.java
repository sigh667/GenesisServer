package com.genesis.common.prop.battleprop.propholder;

import com.genesis.common.prop.battleprop.IPropObserver;
import com.genesis.common.prop.battleprop.propeffect.BattlePropEffect;
import com.genesis.common.prop.battleprop.propeffect.PEIdentifier;
...
public class PartProp {
    ...
    public void resetPropEffect(List<BattlePropEffect> newPropEffects) {
        ...
        for (Entry<PEIdentifier, Integer> entry : changeMap.entrySet()) {
            ...
            changes[index++] =
                    new BattlePropEffect(pEId.getId(), pEId.getEffectType(), entry.getValue());
        }
    ...
    public void remove(HeroBattlePropId propId, PropEffectType type) {
        ...
        while (it.hasNext()) {
            ...
            if (propE.isSameType(propId, type)) {
                it.remove();
                this.notifyChanged(new BattlePropEffect(propE.getPropId(), propE.getType(),
                        -propE.getValue()));
            }
        }
    }
