package com.genesis.common.prop.battleprop.propeffect;
...
public enum PropEffectConverter {
    ...
    public List<BattlePropEffect> convertToBattlePropEffect(TempAttrNode3Col tempAttrNode) {
        ...
        if (tempAttrNode.getAbsValue() != 0) {
            BattlePropEffect effect =
                    new BattlePropEffect(id, PropEffectType.Abs, tempAttrNode.getAbsValue());
            list.add(effect);
        }
        if (tempAttrNode.getPerValue() != 0) {
            BattlePropEffect effect =
                    new BattlePropEffect(id, PropEffectType.Per, tempAttrNode.getPerValue());
            list.add(effect);
        }
    ...
    public BattlePropEffect convertToBattlePropEffect(TempAttrNode2Col tempAttrNode) {
        ...
        if (tempAttrNode.getAttributeValue() != 0) {
            BattlePropEffect effect =
                    new BattlePropEffect(id, PropEffectType.Abs, tempAttrNode.getAttributeValue());
            return effect;
        }
