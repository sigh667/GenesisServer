    ...
    public void check() throws TemplateConfigException {
        if (!ItemType.isValid(this.itemType)) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(),
                    String.format("itemType[%d]无效", this.itemType));
        }

        if (this.itemLevelScope < 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(),
                    String.format("itemLevelScope[%d]不能小于0", this.itemLevelScope));
        }

        if (this.numberMultiple <= 0) {
            throw new TemplateConfigException(this.getSheetName(), this.getId(),
                    String.format("numberMultiple[%d]必须大于0", this.numberMultiple));
        }
    }
