...
public class LogReasonListSqlGenerator {
    ...
    public static void main(String[] args) throws Exception {
        ...
        File output = new File("../tools/src/com/mokylin/bleach/tools/log/log_reason.sql");
        if (output.exists()) {
            output.delete();
        }
    ...
    public static StringBuilder generateSql() throws Exception {
        ...
        for (Class<?> clazz : PackageUtil.getSubClass(packageName, DbLog.class)) {
            ...
            LogReasons.LogDesc _logReason = (LogReasons.LogDesc) _reasonClass.getAnnotation(LogReasons.LogDesc.class);
            ...
            String _tableName = (_className.substring(0, 1).toLowerCase() + _className.substring(1))
            ...
            Enum<?>[] _enums = (Enum<?>[]) invokeStaticMethod(_reasonClass, "values", new Class[0],
                    new Object[0]);
            for (Enum<?> _enum : _enums) {
                LogReasons.ReasonDesc _reasonDesc =
                        _enum.getClass().getField(_enum.name()).getAnnotation(LogReasons.ReasonDesc.class);
                ...
                _reason.reason = ((LogReasons.ILogReason) _enum).getReason() + "";
        ...
        String _insertTemp =
                "insert into reason_list(log_type,log_table,log_desc,log_field,reason,reason_name) values (%s,'%s','%s','%s',%s,'%s');";
        for (SqlReasonDesc _sqlReason : _sqlDescs) {
            ...
            for (SqlReasonDesc.SqlReason _reason : _sqlReason.sqlReasons) {
                _sb.append(String.format(_insertTemp, _sqlReason.type, _sqlReason.tableName,
                        _sqlReason.logDesc, _sqlReason.logField, _reason.reason, _reason.desc));
                _sb.append("\r\n");
            }
