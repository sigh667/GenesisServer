package com.genesis.tools.loggenerator.component;
...
public enum SpecialArg {
    ...
    //本次登录在线时间
    iOnlineTime_acct(Player.class,
            "TimeUnit.MILLISECONDS.toSeconds((timeService.now()-player.getLoginTime()))",
    ...
    iAccumConsume(Human.class,
            "human.getCurrencyManager().get(CurrencyPropId.ACCUMULATED_CHARGE_DIAMOND)",
            CurrencyPropId.class.getName()), //角色累计消耗钻石数
    iMoney(Human.class, "human.getCurrencyManager().get(CurrencyPropId.GOLD)"), //角色身上的金币数
    iGamePoints(Human.class, "human.getCurrencyManager().get(CurrencyPropId.CHARGE_DIAMOND)"),
    //角色登入时拥有钻石数量
    ...
    //本次登录在线时间
    iOnlineTime(Human.class,
            "TimeUnit.MILLISECONDS.toSeconds((timeService.now()-human.getLoginTime().getTime()))"),;
