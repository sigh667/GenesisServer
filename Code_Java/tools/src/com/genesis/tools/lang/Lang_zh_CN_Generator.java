...
public class Lang_zh_CN_Generator {
    ...
    public static void main(String[] args) throws IllegalArgumentException, IllegalAccessException {
        ...
        if (args.length == 1) {
            ...
            LangUtil.createLangExcel(finalDataList, Language.zh_CN.readVersionNumber(),
                    Language.zh_CN.getPath());
        }
     ...
     */
    public static void recordChangeFootprint(List<LangItem> newDataList)
        ...
        Map<String, String> configMap =
                loadConfig(Constants.LANG_FILE_PATH + Constants.CONFIG_FILE_NAME);
        ...
        for (int i = 0; i < newDataList.size(); i++) {
            ...
            if (oldContent == null ||
                    !oldContent.equals(newContent) && !isIgnorModify(ignoreModifyMark)) {
     ...
     */
    private static void recordChangeFootprints(final List<ChangeFootprint> changeFootprints,
            final int curModifyVersion) {
        if (changeFootprints.isEmpty()) {
            return;
        }

        // 记录修改足迹
        ExcelOperation.modifyExcel(Constants.LANG_FILE_PATH + Constants.MODIFY_FOOTPRINT_FILE_NAME,
                new ModifyWorkbookOperation() {
