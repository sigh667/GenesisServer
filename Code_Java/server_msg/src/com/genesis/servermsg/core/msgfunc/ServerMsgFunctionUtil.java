...
public class ServerMsgFunctionUtil {
    ...
    public static ServerMsgFunctionService buildMsgFuncs(String serverMsgFuncsPackage) {
        ...
        Set<Class<?>> funcSet =
                PackageUtil.getSubClass(serverMsgFuncsPackage, IServerMsgFunc.class);

        Table<MessageTarget, Class<?>, IServerMsgFunc<?, MsgArgs, MsgArgs>> table =
                HashBasedTable.create();

        if (funcSet == null || funcSet.isEmpty()) {
        ...
        try {
            for (Class<?> each : funcSet) {
                ...
                IServerMsgFunc<?, MsgArgs, MsgArgs> func =
                        (IServerMsgFunc<?, MsgArgs, MsgArgs>) each.newInstance();
                MessageTarget msgTarget = func.getTarget();
                if (msgTarget == null) {
                    throw new RuntimeException(
                            String.format("Server message class %s don't have a target!",
                                    each.getName()));
                }
                if (table.contains(msgTarget, msgClass)) {
                    throw new RuntimeException("Multi Server Message Func to one Server Message [" +
                            msgClass.getName() + "]");
                }
