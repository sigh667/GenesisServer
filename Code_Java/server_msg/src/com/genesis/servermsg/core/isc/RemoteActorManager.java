...
import com.genesis.servermsg.core.isc.annotation.ISCSync;
import com.genesis.servermsg.core.isc.msg.Connect;
import com.genesis.servermsg.core.isc.msg.ConnectResult;
...
public class RemoteActorManager {
    ...
    public boolean connectRemote(final RemoteServerConfig remoteConfig, String remoteActorPath) {
        ...
        checkArgument(remoteActorPath != null && !remoteActorPath.isEmpty(),
                "RemoteActorManager can not connect to a illegal actor path!");
        ...
        String selectionPath =
                this.buildServerActorPath(remoteConfig.host, remoteConfig.port, remoteActorPath);
        Timeout timeout = new Timeout(Duration.create(5, TimeUnit.SECONDS));

        try {
            Future<Object> future =
                    Patterns.ask(akka.getActorSystem().actorSelection(selectionPath),
                            Connect.INSTANCE, timeout);
            ConnectResult result = (ConnectResult) Await.result(future, timeout.duration());
            if (result == null || result.actorPackages == null) {
                throw new RuntimeException(
                        "RemoteActorManager can not connect to remote cause remote replys null!");
            }
            remoteActorPackages
                    .put(remoteConfig.serverType, remoteConfig.serverId, result.actorPackages);
        } catch (Exception e) {
