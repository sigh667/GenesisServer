...
import com.genesis.servermsg.core.isc.msg.RegisterToRemote;
import com.genesis.servermsg.core.isc.remote.DefaultRemoteFactory;
import com.genesis.servermsg.core.isc.remote.RemoteConnectFunc;
...
import com.genesis.servermsg.core.isc.msg.RegisterToRemoteResult;
import com.genesis.servermsg.core.isc.remote.IRemoteFactory;
import com.genesis.servermsg.core.isc.remote.actorrefs.SingleTargetActorRef;
...
public class ISCActorSupervisor extends UntypedActor {
    ...
    public ISCActorSupervisor(ServerConfig localConfig, ISCService iscService,
            String serverMsgFuncsPackage) {
        this(localConfig, iscService, serverMsgFuncsPackage, new DefaultRemoteFactory());
    }

    public ISCActorSupervisor(ServerConfig localConfig, ISCService iscService,
            String serverMsgFuncsPackage, IRemoteFactory remoteFactory) {
        this(localConfig, iscService, serverMsgFuncsPackage, remoteFactory, 1);
    }

    public ISCActorSupervisor(ServerConfig localConfig, ISCService iscService,
            String serverMsgFuncsPackage, IRemoteFactory remoteFactory, int defaultThreadNum) {
        this.localConfig = checkNotNull(localConfig,
                "ISCActorSupervisor can not init with null server config!");
        this.iscService =
                checkNotNull(iscService, "ISCActorSupervisor can not init with null ISCService!");
        localActorPackage =
                new SingleTargetActorRef(this.localConfig.serverType, this.localConfig.serverId,
                        getContext().actorOf(Props.create(ISCServerActor.class, this.iscService,
                                serverMsgFuncsPackage, remoteFactory, defaultThreadNum),
                                ISCServerActor.ACTOR_NAME));
    }

    public SupervisorStrategy supervisorStrategy() {
        return new OneForOneStrategy(SupervisorStrategy
                .makeDecider(new Function<Throwable, SupervisorStrategy.Directive>() {
                    @Override
                    public Directive apply(Throwable e) throws Exception {
                        log.error("Actor {} error arised.",
                                ISCActorSupervisor.this.getSelf().path().toString(), e);
