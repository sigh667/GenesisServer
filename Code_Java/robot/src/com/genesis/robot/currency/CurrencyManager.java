package com.genesis.robot.currency;
...
public class CurrencyManager {
    ...
    /** 金币属性管理器 */
    protected EnumArray<Currency, Long> currencyProp =
            EnumArray.create(Currency.class, Long.class, 0L);
    private Human human;
    ...
    public void load(List<Long> currencyPropList) {
        ...
        if (currencyIds.length != currencyPropList.size()) {
            String error = String.format(
                    "Currency count==[%d],msg's currencyPropList count==[%d],not equal!",
    ...
    public boolean isMoneyEnough(Currency currency, long value) {
        if (currency == null || value <= 0) {
            throw new IllegalArgumentException(
                    String.format("参数有误：value=%d (value必须大于0), currency=%s (currency不能为空)", value,
                            currency));
        }
