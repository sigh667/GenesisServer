package com.genesis.gamedb.uuid;
...
public enum UUIDType implements IUUIDType {
    /**角色表*/
    Human(HumanEntity.class), /**道具表*/
    Item(ItemEntity.class), /** 商店表 */
    Shop(ShopEntity.class), /** 商店打折 */
    ShopDiscount(ShopDiscountEntity.class), /** 功能 */
    ...
    public long qurryOldMaxUuidFromDB(HibernateDBService dbService, int serverGroup, int serverId) {
        ...
        final String _sql =
                String.format("select max(id) from %s where id >=:minId and id <= :maxId",
                        _className);
        List<? extends BaseEntity> entityList = dbService
                .findBySqlQueryAndParamAllT(entityClass, _sql, new String[]{"minId", "maxId"},
                        new Object[]{scope.minValue, scope.maxValue});
