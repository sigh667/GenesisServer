package com.genesis.gameserver.hero.star;
...
public class HeroStarService {
    ...
    public void init(TemplateService templateservice) {
        ...
        final Map<Integer, HeroGrowStarTemplate> allMap =
                templateservice.getAll(HeroGrowStarTemplate.class);
        ...
        for (int i = firstKey; i <= lastKey; i++) {
            if (!treeMap.containsKey(i)) {
                String error =
                        String.format("Hero最低星级为【%d】最高星级为【%d】,但中间星级【%d】的属性却未配置", firstKey, lastKey,
                                i);
        ...
        for (HeroGrowTemplate heroGrowTemplate : growMap.values()) {
            ...
            for (TempAttrNode2Col tempAttrNode2 : attributeList) {
                BattlePropEffect effect =
                        PropEffectConverter.Inst.convertToBattlePropEffect(tempAttrNode2);
                if (effect == null) {
                    continue;
                }
            ...
            for (int i = minStarCount; i <= maxStarCount; i++) {
                ...
                HeroGrowStarTemplate heroGrowStarTemplate =
                        GlobalData.getTemplateService().get(i, HeroGrowStarTemplate.class);
                float growthRate = heroGrowStarTemplate.getGrowthRate();
                for (BattlePropEffect basePropEffect : baseEffectList) {
                    BattlePropEffect effect = new BattlePropEffect(basePropEffect.getPropId(),
                            basePropEffect.getType(),
