...
import com.genesis.gameserver.shop.timeevent.ShopAutoRefresh;
import com.genesis.gameserver.shop.timeevent.ShopManuallyRefresh;
...
public class ShopRefresh {
    ...
    public ShopRefresh(Shop shop, Human human) {
        ...
        Timestamp lastResetManuallyRefreshCountTime =
                new Timestamp(TimeUtils.TIMESTAMP_INITIAL_VALUE);
        this.mauallyRefresh =
                new ShopManuallyRefresh(shop.getShopType().getAutoResetManuallyRefreshTime(),
                        human.getTimeAxis(), shop, 0, lastResetManuallyRefreshCountTime);

        List<LocalTime> autoRefreshTimes =
                Globals.getShopRelatedTemplateBufferData().getAutoRefreshTime(shop.getShopType());
        Timestamp lastAutoRefreshTime = new Timestamp(TimeUtils.TIMESTAMP_INITIAL_VALUE);
        this.autoRefresh = new ShopAutoRefresh(autoRefreshTimes, lastAutoRefreshTime, shop,
                human.getTimeAxis());
    }
    ...
    public ShopRefresh(Shop shop, Human human, int manuallyRefreshCount,
        ...
        this.mauallyRefresh =
                new ShopManuallyRefresh(shop.getShopType().getAutoResetManuallyRefreshTime(),
                        human.getTimeAxis(), shop, manuallyRefreshCount,
                        lastResetManuallyRefreshCountTime);
    ...
    public void refresh(int humanLevel, long now) {
        List<ShopGoodTemplate> shopGoodTmpls = Globals.getShopRelatedTemplateBufferData()
                .getShopGoodTmplList(this.shop.getShopType());
    ...
    private Good randomSelectGood(ShopGoodTemplate shopGoodTmpl, int humanLevel) {
        ...
        int critRate = RandomUtil.randomSuccess(shopGoodTmpl.getCritChance()) ?
                shopGoodTmpl.getCritRate() : 1;
        ShopDiscount shopDiscount =
                this.discountService.getShopDiscount(shopGoodTmpl.getShopType());
        ...
        int shopDiscountNumMultiple =
                discounting ? shopDiscount.getNumMultiple() : ShopDiscountService.NO_MULTIPLE;
        int finalGoodNum =
                shopGoodTmpl.getBaseNum() * critRate * storeRoomTmpl.getNumberMultiple() *
                        shopDiscountNumMultiple;
        if (finalGoodNum <= 0) {
            throw new RuntimeException(
                    String.format("随机刷出的商品数量必须大于0！【ShopGoodTemplateId:%d】【humanLevel:%d】",
                            shopGoodTmpl.getId(), humanLevel));
        }
