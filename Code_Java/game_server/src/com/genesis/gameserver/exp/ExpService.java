package com.genesis.gameserver.exp;
...
public class ExpService {
    ...
    public static long getExpByLevel(long level, ExpTemplate.ExpEnum expType) {
        ...
        checkArgument(level > 0 || level <= getCachedSize(),
                String.format("level=%d (level必须>0, 且<=已定义的最高等级 %d)", level, expMap.size()));
    ...
    public boolean isAddable(long currLevel, long topLevel, long currExp,
        ...
        if (currLevel > Integer.MAX_VALUE || topLevel > Integer.MAX_VALUE) {
            throw new IllegalArgumentException(String.format(
                    "currLevel=%d (currLevel必须>0, 且<=topLevel), topLevel=%d (topLevel必须>0, 且<=Integer.MAX_VALUE)",
                    currLevel, topLevel));
        }
     ...
     * @return ExpData  level和exp
     */
    private ExpData addExp(int currLevel, int topLevel, long currExp, long addExp,
            ExpTemplate.ExpEnum expType) {

        if (expMap == null || expType == null || currExp < 0 || addExp < 0 ||
                MathUtils.longAddOverflow(currExp, addExp) || currLevel > Integer.MAX_VALUE ||
                topLevel > Integer.MAX_VALUE || currLevel <= 0 || currLevel > topLevel ||
                currLevel > getCachedSize()) {
            throw new IllegalArgumentException(String.format(
                    "参数有误：currLevel=%d (currLevel必须>0, 且<=topLevel), topLevel=%d (topLevel必须>0, 且<=Integer.MAX_VALUE), currExp=%d (currExp必须>=0), addExp=%d (addExp必须>=0), expType=%s (expType不能为null)",
    ...
    private boolean isAddable(int currLevel, int topLevel, long currExp,
        ...
        if (currExp < 0 || currLevel <= 0 || currLevel > topLevel ||
                currLevel > Integer.MAX_VALUE || topLevel > Integer.MAX_VALUE || expType == null ||
                expMap == null || currLevel > getCachedSize()) {
            throw new IllegalArgumentException(String.format(
                    "currLevel=%d (currLevel必须>0, 且<=topLevel), topLevel=%d (topLevel必须>0, 且<=Integer.MAX_VALUE), currExp=%d (currExp必须>=0), expType=%s (expType不能为null)",
