...
public class DataUpdater implements IDataUpdater {
     ...
     */
    private HashMap<PersistanceWrapper, WrapperData> map =
            new HashMap<PersistanceWrapper, WrapperData>();

    public DataUpdater(String hostName, int maxSizeToUpdate, long millisSecond,
    ...
    public void heartbeat() {
        if (isNeedFlush()) {
            ...
            if (flushDuaration > TIME_VALVE) {
                logger.warn(hostName + " .update 数量" + count + " 时间过长：" +
                        TimeUnit.NANOSECONDS.toMillis(flushDuaration) + "ms");
            } else if (flushDuaration > 0 && logger.isDebugEnabled()) {
                logger.debug("[#GS.CommonDataUpdater.flush] [Update Time:" +
                        TimeUnit.NANOSECONDS.toMillis(flushDuaration) + "ms]");
            }
        }
    }
    ...
    public void flush() {
        ...
        for (Iterator<Entry<PersistanceWrapper, WrapperData>> iterator = map.entrySet().iterator();
                iterator.hasNext(); ) {
            ...
            try {
                ...
                logger.error(
                        "DataUpdater:flush() entry.getKey().getSqlObject().toEntity() throw Exception!",
                        e);
                iterator.remove();
                continue;
            }
            ...
            try {
                ...
                logger.error("DataUpdater:flush() dirtyData.entity.newRedisKey() throw Exception!",
                        e);
                iterator.remove();
                continue;
            }
    ...
    private void add(PersistanceWrapper wrapper, DbOp op) {
        if (wrapper == null) {
            logger.warn(
                    "CommonDataUpdater:add(PersistanceWrapper wrapper, DbOp op) wrapper is null!");
            return;
        }
        ...
        if (!map.containsKey(wrapper)) {
            ...
            if (oldWrapper != wrapper) {
                logger.warn("持久化时，包装类不同但却指向同一ID的数据Class=【{}】ID=【{}】",
                        oldWrapper.getSqlObject().getClass().getName(),
                        oldWrapper.getSqlObject().getDbId());
                flush();
