package com.genesis.gameserver.core.persistance.task;
...
public class PersistanceIntoRedisTask implements ArgsRunnable {
    ...
    public void run(AsyncArgs args) {
        ...
        @SuppressWarnings("unchecked") final HashMap<Long, Integer> human2originalServerId =
                (HashMap<Long, Integer>) args.get(HumanInfoCache.HUMAN_TO_ORIGINAL_SERVER_ID);
        pipeline.exec(new PipelineProcess() {
            ...
            public void apply() {
                for (DirtyData dirtyData : dirtyList) {
                    try {
                        IRedisKey<? extends Serializable, ? extends EntityWithRedisKey<?>>
                                redisKey = dirtyData.getDirtyDataInfo().getRedisKey();
                        DbOp dbOp = dirtyData.getDirtyDataInfo().getDbOp();

                        if (dbOp == DbOp.DELETE) {
                            redisKey.getEntityRedisOp().deleteFromRedis(this, redisKey);
                        } else {
                            @SuppressWarnings("unchecked") IRedisKey<? extends Serializable, EntityWithRedisKey<?>>
                                    redisKeyTmp =
                                    (IRedisKey<? extends Serializable, EntityWithRedisKey<?>>) redisKey;
                            redisKeyTmp.getEntityRedisOp()
                                    .flushToRedis(this, redisKeyTmp, dirtyData.getEntity());
                        }

                        int originalServerId = Mapping.INVALID_SERVER_ID;
                        if (dirtyData.getEntity() instanceof IHumanRelatedEntity) {
                            //Human相关的数据
                            IHumanRelatedEntity iHumanRelatedEntity =
                                    (IHumanRelatedEntity) dirtyData.getEntity();
                            originalServerId = getOriginalServerId(iHumanRelatedEntity.humanId());

                            Map<Integer, Integer> gs_dbs_Map =
                                    Globals.getServerMappings().getGs_dbs_Map();
                            if (!gs_dbs_Map.containsKey(originalServerId)) {
                                logger.warn("originalServerId【{}】 不存在，EntityClass【{}】，humanId【{}】",
                                        originalServerId,
                                        dirtyData.getEntity().getClass().getName(),
                                        iHumanRelatedEntity.humanId());
                                continue;
                            }
                        } else if (dirtyData.getEntity() instanceof IServerRelatedEntity) {
                            //服务器相关的数据
                            IServerRelatedEntity iServerRelatedEntity =
                                    (IServerRelatedEntity) dirtyData.getEntity();
                        ...
                        String dirtyDataKey =
                                SpecialRedisKeyBuilder.buildDirtyDataKey(originalServerId);
                        this.getListOp().rpush(dirtyDataKey, dirtyData.getDirtyDataInfo());
                    } catch (Exception e) {
                        logger.error(
                                "PersistanceIntoRedisTask flush to redis runnable throw Exception!",
                                e);
                        continue;
                    }
                }
