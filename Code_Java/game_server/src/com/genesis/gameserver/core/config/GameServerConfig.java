...
public class GameServerConfig {
    ...
    private static Config config = ConfigBuilder.buildConfigFromFileName("GameServer.conf");
    ...
    public GameServerConfig(ServerConfig sConfig, int group, String redis,
            ServerConfig agentServerConfig, int[] dataServerIds, boolean isOpen,
            boolean isLocalLoginAllowed) {
    ...
    public static LogConfig getLogConfig() {
        ...
        return new LogConfig(logConfig.getString("scribeHost"), logConfig.getInt("scribePort"),
                logConfig.getLong("flushTimeGap"), logConfig.getLong("cacheMaxNum"),
                logConfig.getLong("flushTimeAccuracy"), mockSendLog);
    }
    ...
    public static List<ServerConfig> getConnectedDataConfig() {
        Collection<ConfigValue> dataServerList =
                config.getConfig("server.connectTo.dataServers").root().values();
    ...
    public static List<GameServerConfig> getGameServerConfig() {
        Collection<ConfigValue> gameServerList =
                config.getConfig("server.gameServers").root().values();
        ...
        for (ConfigValue each : gameServerList) {
            ...
            if (co.get("localLogin") != null) {
                isLocalLoginAllowed = (boolean) co.get("localLogin").unwrapped();
            }
    ...
    private static void assertDataServerConfigured(List<Integer> dataServerIds, int gameServerId,
            List<ServerConfig> dataConfigs) {
        if (dataServerIds == null || dataServerIds.isEmpty()) {
            throw new RuntimeException(
                    "GameServer: [" + gameServerId + "] can not started without data server!");
        }
        for (int eachId : dataServerIds) {
            ...
            if (!bFlag) {
                throw new RuntimeException(
                        "DataServerID: [" + eachId + "] configured in GameServer: [" +
                                gameServerId + "] has been not configured before!");
            }
        }
    }
