...
public class ServerInitUtil {
    ...
    public static Map<Class<?>, ServerInitFunction<?>> buildServerInitFunction() {
        Set<Class<?>> funcClassSet =
                PackageUtil.getSubClass("com.mokylin.bleach.gameserver", ServerInitFunction.class);
        ...
        try {
            for (Class<?> each : funcClassSet) {
                ...
                map.put(extractInitObjectType(each, ServerInitFunction.class),
                        (ServerInitFunction<?>) each.newInstance());
            }
        } catch (Exception e) {
