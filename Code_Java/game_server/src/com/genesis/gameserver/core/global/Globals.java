...
import com.genesis.core.isc.remote.actorrefs.ActorPackagesUtil;
import com.genesis.core.msgfunc.MsgArgs;
import com.genesis.core.msgfunc.client.ClientMsgFunctionService;
import com.genesis.core.msgfunc.client.ClientMsgFunctionUtil;
import com.genesis.core.msgfunc.client.IClientMsgFunc;
import com.genesis.core.msgfunc.server.ServerMsgFunctionService;
import com.genesis.core.msgfunc.server.ServerMsgFunctionUtil;
...
import com.genesis.gameserver.chat.cmd.core.GmCmdFunctionService;
import com.genesis.gameserver.chat.cmd.core.GmCmdFunctionUtil;
import com.genesis.gameserver.core.concurrent.GameProcessUnit;
import com.genesis.gameserver.core.concurrent.GameScheduledProcessUnit;
...
import com.genesis.gameserver.core.config.LogConfig;
...
import com.genesis.gameserver.core.log.disruptor.sendlog.MockSendLog;
import com.genesis.gameserver.core.log.disruptor.sendlog.SendLogManager;
import com.genesis.gameserver.core.redis.ActionOnCannotConnectRedis;
import com.genesis.gameserver.core.serverinit.ServerInitFuncService;
import com.genesis.gameserver.core.serverinit.ServerInitUtil;
import com.genesis.gameserver.core.timeevent.TimeEventService;
import com.genesis.gameserver.exp.ExpService;
import com.genesis.gameserver.hero.equip.EquipService;
import com.genesis.gameserver.hero.group.HeroGroupService;
import com.genesis.gameserver.hero.star.HeroStarService;
import com.genesis.gameserver.item.ItemRelatedTemplateBufferData;
import com.genesis.gameserver.shop.ShopRelatedTemplateBufferData;
...
public class Globals {
    ...
    public static void init() {
        ...
        if (logConfig.isMockSendLog()) {
            ...
            sendLog = new SendLogManager(logConfig.getScribeHost(), logConfig.getScribePort(),
                    logConfig.getFlushTimeGap(), logConfig.getCacheMaxNum(),
                    logConfig.getFlushTimeAccuracy());
        }

        redisService = new RedisService(new ActionOnCannotConnectRedis(),
        ...
        Pair<Table<MessageTarget, Integer, IClientMsgFunc<GeneratedMessage, MsgArgs, MsgArgs>>, Map<Integer, Parser<? extends GeneratedMessage>>>
                pair = ClientMsgFunctionUtil.buildMsgFunction("com.mokylin.bleach.gameserver");
        ...
        targetClassMap =
                ActorPackagesUtil.buildTargetClassMap("com.mokylin.bleach.gameserver", false);
        serverInitProcessUnit =
                GameServerProcessUnitHelper.createFixedProcessUnit("server init", 3);
        platformAuthProcessUnit =
                GameServerProcessUnitHelper.createFixedProcessUnit("local auth", 50);
        scheduleProcessUnit =
                GameServerProcessUnitHelper.createScheduleProcessUnit("schedule", 1, akka);

        scheduleProcessUnit
                .schedule(FiniteDuration.Zero(), FiniteDuration.create(100, TimeUnit.MILLISECONDS),
                        new Runnable() {
                            @Override
                            public void run() {
