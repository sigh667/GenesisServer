...
public class TimeEventService {
     ...
     */
    public <T> void startPeriodEventInRecentEventTime(TimeAxis<T> timeAxis, long now,
            LocalTime time, long period, ITimeEvent<T> event) {
    ...
    private long getEventTimeOfToday(long now, int millisOfDay) {
        return new DateTime(Globals.getTimeService().now()).withMillisOfDay(millisOfDay)
                .getMillis();
    }
     ...
     */
    public <T> void startPeriodEventInRecentEventTime(TimeAxis<T> timeAxis, long now,
            long startTime, long period, ITimeEvent<T> event) {
        if (event == null) {
            return;
        }
