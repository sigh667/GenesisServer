package com.genesis.gameserver.core.timeout;
...
public class TimeoutCallbackManager {
    ...
    public void startSchedule(final long timeoutCtxId, long delay, TimeUnit timeUnit) {
        ...
        Cancellable future = Globals.getScheduleProcessUnit()
                .scheduleOnce(old.timeoutCB.getExecuteActor(), old, delay, timeUnit);

        if (timeoutMap.replace(timeoutCtxId,
                new TimeoutCBWrapper(old.timeoutId, old.timeoutCB, future)) == null) {
            future.cancel();
        }
    }
