package com.genesis.gameserver.item;
...
public class Inventory extends ItemContainer<Human, Item> {
        ...
        for (ItemEntity itemEntity : itemEntityList) {
            ...
            if (item.getOverlap() <= 0) {
                throw new RuntimeException(
                        "Player: [" + human.getId() + "] has Item: [" + item.getTemplateId() +
                                "], but the amount is 0!");
            }
        }
    }
    ...
    public void notifyOnLogin() {
        com.genesis.protobuf.ItemMessage.GCInventoryInfo.Builder inventoryBuilder =
    ...
    public boolean addItem(int itemTemplateId, int amount) {
        ...
        if (item == null) {
            ItemTemplate itemTmpl =
                    GlobalData.getTemplateService().get(itemTemplateId, ItemTemplate.class);
            if (itemTmpl == null) {
                return false;
            }
    ...
    public boolean deleteItem(int itemTemplateId, int amount) {
        ...
        if (amount <= 0 || amount > item.getOverlap()) {
            return false;
        }
    ...
    public boolean batchDeleteItems(ItemInfo... items) {
        ...
        for (int i = 0; i < allItems.length; i++) {
            ...
            if (item.getOverlap() < temp.amount) {
                return false;
            }
