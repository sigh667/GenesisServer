...
//import LoginService;
//import OnlinePlayerService;
//import LoginLogger;
//import PlatformAuthResult;
...
// */
//
//public class PlatformAuthResultFunc
//        implements IServerMsgFunc<PlatformAuthResult, ServerGlobals, PlayerManagerArgs> {
//
//    private final static Logger log = LoginLogger.log;
//
//    @Override
//    public void handle(IRemote remote, PlatformAuthResult msg, ServerGlobals sGlobals,
...
//            log.warn(
//                    "Login Error! Can not find login player after local authed, Channel: [{}], AccountId: [{}], AgnetSessionId: [{}]",
...
//             */
//            log.warn(
//                    "Login Error! The authed Player and current login Player is not the same object. Channel: [{}], AccountId: [{}]",
//                    channel, accountId);
//            return;
//        }
//
...
//            //玩家已经下线了，此时移除该玩家即可
//            playerManagerArgs.loginService
...
//            log.warn(
//                    "Login Error! The LoginStatus should be Authing but has been damaged. Channel: [{}], AccountId: [{}], LoginStatus: [{}]",
...
//            msg.player.sendMessage(
//                    GCLoginFail.newBuilder().setAccountId(msg.accountId).setChannel(msg.channel)
//                            .setKey(msg.key).setFailReason(LoginFailReason.KEY_WRONG));
//            msg.player.disconnect();
//            log.warn(
//                    "Login Failed! Authentication Failed. Channel: [{}], AccountId: [{}], LoginStatus: [{}]",
//                    channel, accountId, msg.player.getStatus().name());
//            return;
//        }
//
...
//            Player alreadyExistPlayer =
//                    playerManagerArgs.onlinePlayerService.getPlayer(msg.channel, msg.accountId)
//                            .get();
...
//    }
//
//    private void bothKickOut(Player currentPlayer, Player alreadyExistPlayer,
//            PlatformAuthResult msg) {
//        Builder loginFailBuilder =
//                GCLoginFail.newBuilder().setAccountId(msg.accountId).setChannel(msg.channel)
//                        .setKey(msg.key).setFailReason(LoginFailReason.YOUR_ACCOUNT_IS_ONLINE);
...
//    }
//
//    private void loadHumans(Player player, PlatformAuthResult msg, ServerGlobals sGlobals,
//            PlayerManagerArgs playerManagerArgs) {
...
//                roleListBuilder.addRoles(
//                        Role.newBuilder().setId(humanInfo.getId()).setName(humanInfo.getName()));
//            }
...
//                GCLoginFail msgToSend =
//                        builder.setAccountId(msg.accountId).setChannel(msg.channel).setKey(msg.key)
...
//                    new LoadHumanDataTask(humanInfo, player, sGlobals, playerManagerArgs));
//            return;
//        }
//    }
//
//    private boolean hasPlayerAlreadyLogined(PlatformAuthResult msg,
//            PlayerManagerArgs playerManagerArgs) {
//        OnlinePlayerService onlinePlayerService = playerManagerArgs.onlinePlayerService;
//        Optional<Player> alreadyExistPlayerOption =
//                onlinePlayerService.getPlayer(msg.channel, msg.accountId);
