package com.genesis.gameserver.login.task;
...
import com.genesis.gameserver.login.callback.LoadHumanDataFromDBSTimeOutCallBack;
...
public class LoadHumanDataTask implements ArgsRunnable {
    ...
    public LoadHumanDataTask(HumanInfo humanInfo, Player player, ServerGlobals sGlobals,
    ...
    public void run(AsyncArgs args) {
        ...
        if (player.getStatus() == LoginStatus.Logouting) {
            playerManagerArgs.context.self()
                    .tell(new LoadHumanDataAborted(player), ActorRef.noSender());
            return;
        }
        ...
        try {
            ...
            if (humanEntity == null) {
                ...
                long timeoutId = this.timeoutCBManager.registerTimeoutCB(
                        new LoadHumanDataFromDBSTimeOutCallBack(player, humanInfo,
                                this.playerManagerArgs));
                LoadHumanDataMessage msg = new LoadHumanDataMessage(timeoutId, player.getChannel(),
                        player.getAccountId(), this.humanInfo.getId(),
                        this.humanInfo.getOriginalServerId());
                ...
                playerManagerArgs.context.self()
                        .tell(new LoadHumanDataSuccess(humanData, player), ActorRef.noSender());
            }
        } catch (Throwable t) {
            log.error(
                    "Load human data error occured, Server: [{}], Player: [{}|{}], Channel: [{}], Error: {}",
                    serverId, player.getAccountId(), this.humanInfo.getId(), player.getChannel(),
                    t);
            playerManagerArgs.context.self()
                    .tell(new LoadHumanDataFailed(player), ActorRef.noSender());
        }
    }
