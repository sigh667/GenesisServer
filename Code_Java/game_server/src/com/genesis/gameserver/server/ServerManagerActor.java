...
public class ServerManagerActor extends UntypedActor {
    ...
    public ServerManagerActor(List<GameServerConfig> configs) {
        ...
        for (GameServerConfig config : configs) {
            ...
            ActorRef newServer = this.getContext().actorOf(Props.create(ServerActor.class, config),
                    String.valueOf(config.serverConfig.serverId));
                ...
                .makeDecider(new Function<Throwable, SupervisorStrategy.Directive>() {
                    ...
                    public Directive apply(Throwable e) throws Exception {
                        log.error("Actor {} error arised.",
                                ServerManagerActor.this.getSelf().path().toString(), e);
    ...
    private void startNewServer(StartNewServer msg) {
        if (msg.newServerConfig == null) {
            ...
            ActorRef newServer = this.getContext()
                    .actorOf(Props.create(ServerActor.class, msg.newServerConfig),
                            String.valueOf(msg.newServerConfig.serverId));
