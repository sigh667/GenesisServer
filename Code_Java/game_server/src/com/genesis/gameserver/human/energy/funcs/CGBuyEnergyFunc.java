package com.genesis.gameserver.human.energy.funcs;
...
import com.genesis.protobuf.HumanMessage.BuyEnergyFail;
import com.genesis.protobuf.HumanMessage.CGBuyEnergy;
import com.genesis.protobuf.HumanMessage.GCBuyEnergyAck;
import com.genesis.protobuf.HumanMessage.GCBuyEnergyNeg;
...
public class CGBuyEnergyFunc extends AbstractClientMsgFunc<CGBuyEnergy, Human, ServerGlobals> {
    ...
    public void handle(Player player, CGBuyEnergy msg, Human human, ServerGlobals sGlobals) {
        ...
        if (buyEnergyCounts > maxCounts) {
            human.sendMessage(
                    GCBuyEnergyNeg.newBuilder().setFailType(BuyEnergyFail.FAIL_OVER_MAX_TIMES));
            return;
        }
        ...
        if (!human.getEnergyManager().isAddable()) {
            human.sendMessage(
                    GCBuyEnergyNeg.newBuilder().setFailType(BuyEnergyFail.FAIL_OVER_MAX_VALUES));
            return;
        }
        //3. 检查钻石是否足够
        ...
        if (!human.isMoneyEnough(Currency.DIAMOND, costDiamond)) {
            human.sendMessage(
                    GCBuyEnergyNeg.newBuilder().setFailType(BuyEnergyFail.FAIL_DIAMOND_NOT_ENOUGH));
            return;
        }
        ...
        human.sendMessage(GCBuyEnergyAck.newBuilder().setAddedEnergy(addedEnergy)
                .setCurrencyChangeInfo(currencyChangeInfo));
    }
    ...
    private int getMaxBuyCounts(Human human) {
        VipPrivilege privilege =
                VipHelper.getVipPrivilege(human, VipPrivilegeType.BUY_ENERGY_COUNTS);
        //权限未开启则返回0
        if (!privilege.isOpen()) {
            return 0;
        }
