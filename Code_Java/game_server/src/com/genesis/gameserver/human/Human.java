package com.genesis.gameserver.human;
...
import com.genesis.gameserver.currency.CurrencyManager;
import com.genesis.gameserver.dailyreward.DailyRewardManager;
import com.genesis.gameserver.function.FunctionManager;
import com.genesis.gameserver.hero.HeroManager;
import com.genesis.gameserver.human.energy.EnergyManager;
import com.genesis.gameserver.human.event.HumanLevelUpEvent;
import com.genesis.gameserver.human.event.HumanLevelUpReason;
import com.genesis.gameserver.item.Inventory;
import com.genesis.gameserver.player.Player;
import com.genesis.gameserver.shop.ShopManager;
import com.genesis.gameserver.window.WindowManager;
...
import com.genesis.protobuf.HumanMessage.GCHumanDetailInfo.Builder;
import com.genesis.protobuf.HumanMessage.GCHumanLevelUp;
...
public class Human extends ObjectInSqlImpl<Long, HumanEntity> implements MsgArgs, IPropNotifier {
     ...
     * @param sGlobals
     */
    public Human(HumanInfo humanInfo, Player player, IDataUpdater dataUpdater,
     ...
     * @param sGlobals
     */
    public Human(HumanData humanData, Player player, DataUpdater dataUpdater,
            ServerGlobals sGlobals) {
        super(dataUpdater);
    ...
    public HumanEntity toEntity() {
        ...
        humanEntity.setAccumulatedChargeDiamond(
                this.currencyManager.get(CurrencyPropId.ACCUMULATED_CHARGE_DIAMOND));
        humanEntity.setAccumulatedConsumedChargeDiamond(
                this.currencyManager.get(CurrencyPropId.ACCUMULATED_CONSUMED_CHARGE_DIAMOND));
        ...
        humanEntity.setLastEnergyRecoverTime(
                new Timestamp(this.propContaniner.get(HumanPropId.LAST_ENERGY_RECOVER_TIME)));
        humanEntity.setBuyEnergyCounts(this.propContaniner.get(HumanPropId.BUY_ENERGY_COUNTS));
        humanEntity.setLastBuyEnergyCountsResetTime(
                new Timestamp(propContaniner.get(HumanPropId.LAST_BUY_ENERGY_COUNTS_RESET_TIME)));
        return humanEntity;
    }
    ...
    public void fromEntity(HumanEntity entity) {
        ...
        if (entity.getChargeDiamond() < 0 || entity.getFreeDiamond() < 0 ||
                MathUtils.longAddOverflow(entity.getChargeDiamond(), entity.getFreeDiamond())) {
            throw new RuntimeException(
                    "充值钻石和免费钻石数据错误：充值钻石[" + entity.getChargeDiamond() + "], 免费钻石[" +
                            entity.getFreeDiamond() + "]");
        }
        this.currencyManager.set(CurrencyPropId.ACCUMULATED_CHARGE_DIAMOND,
                entity.getAccumulatedChargeDiamond());
        this.currencyManager.set(CurrencyPropId.ACCUMULATED_CONSUMED_CHARGE_DIAMOND,
                entity.getAccumulatedConsumedChargeDiamond());
        ...
        this.propContaniner.set(HumanPropId.LAST_ENERGY_RECOVER_TIME,
                entity.getLastEnergyRecoverTime().getTime());
        this.propContaniner.set(HumanPropId.BUY_ENERGY_COUNTS, entity.getBuyEnergyCounts());
        this.propContaniner.set(HumanPropId.LAST_BUY_ENERGY_COUNTS_RESET_TIME,
                entity.getLastBuyEnergyCountsResetTime().getTime());
    ...
    public boolean isExpAddable() {
        return Globals.getExpService()
                .isAddable(this.get(HumanPropId.LEVEL), GlobalData.getConstants().getMaxLevel(),
    ...
    public void addExp(long addExp, HumanLevelUpReason reason) {
        ...
        ExpData exp = Globals.getExpService()
                .addExp(this.get(HumanPropId.LEVEL), GlobalData.getConstants().getMaxLevel(),
                        this.get(HumanPropId.EXP), addExp, ExpTemplate.ExpEnum.HUMAN);
