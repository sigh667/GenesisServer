...
import com.genesis.gameserver.core.actor.StopSupervisorStrategyActor;
...
import com.genesis.gameserver.core.log.LogOnlineCountsTask;
import com.genesis.gameserver.login.protocol.RemoveHumanInfoMsg;
...
public class PlayerManagerActor extends StopSupervisorStrategyActor {
    ...
    public PlayerManagerActor(ServerGlobals sGlobals, HumanInfoCache humanInfoCache) {
        this.sGlobals = checkNotNull(sGlobals,
                "PlayerManagerActor can not be created with null ServerGlobals!");
        checkNotNull(humanInfoCache,
                "PlayerManagerActor can not be created with null human info cache!");
        onlinePlayerService = new OnlinePlayerService(sGlobals);
        playerActorRefs = new HashMap<>();
        args = new PlayerManagerArgs(this.getContext(), humanInfoCache, onlinePlayerService,
                playerActorRefs);
        timeAxis = new TimeAxis<PlayerManagerActor>(Globals.getTimeService(), this);

        //记录日志周期
        long millis =
                TimeUnit.SECONDS.toMillis(GlobalData.getConstants().getLogOnlineCountsPeriod());
        //运营日志-调度定时任务，记录玩家在线日志
        timeAxis.scheduleEventAfterMS(new LogOnlineCountsTask(sGlobals, onlinePlayerService),
                millis, millis);
    }
    ...
    public void onReceive(Object msg) throws Exception {
        if (msg instanceof ServerMessage) {
            ...
            Globals.getServerMsgFuncService()
                    .handle(MessageTarget.PLAYER_MANAGER, sGlobals.getAgentServer(), msg, sGlobals,
                            args);
        }
    }
    ...
    private void handleServerMessage(ServerMessage sMsg) {
        if (sMsg.msg instanceof CSSMessage) {
            ...
            if (playerOption.isPresent()) {
                ...
                log.warn(
                        "PlayerManagerActor receive a message [{}] can not find player session: AgentServerId:[{}], ServerType:[{}], ServerId:[{}]",
            ...
            if (option.isPresent()) {
                Globals.getServerMsgFuncService()
                        .handle(MessageTarget.PLAYER_MANAGER, option.get(), sMsg.msg, sGlobals,
                                args);
            } else {
                log.warn(
                        "PlayerManagerActor receive a message [{}] can not find remote: ServerType:[{}], ServerId:[{}]",
    ...
    private void handleCSSMsg(CSSMessage msg, Player player) {
        if (player.getStatus().isCanProcess(msg.messageType)) {
            try {
                Globals.getClientMsgFuncService()
                        .handle(MessageTarget.PLAYER_MANAGER, msg.messageType, msg.messageContent,
                                player, sGlobals, args);
            } catch (Exception e) {
                log.error(
                        "Exception occurs in PlayerManagerActor: Account [{}], Channel [{}] Exception [{}].",
                        player.getAccountId(), player.getChannel(), e);
                player.disconnect();
            }
        } else {
            log.warn(
                    "PlayerManagerActor: Wrong message [{}] is received in Player Status: [{}], Player session id: [{}]",
