...
import com.genesis.gameserver.core.heartbeat.PlayerHeartbeat;
...
public class PlayerActorNormalFunc implements Procedure<Object> {
    ...
    public PlayerActorNormalFunc(PlayerActor playerActor, Human human, Player player,
            ServerGlobals sGlobals, DataUpdater dataUpdater) {
        ...
        if (sMsg.msg instanceof CSSMessage) {
            ...
            if (option.isPresent()) {
                Globals.getServerMsgFuncService()
                        .handle(MessageTarget.PLAYER, option.get(), sMsg.msg, human, sGlobals);
            } else {
                log.warn(
    ...
    private void handleCSSMsg(CSSMessage msg, Player player) {
        if (player.getStatus().isCanProcess(msg.messageType)) {
            Globals.getClientMsgFuncService()
                    .handle(MessageTarget.PLAYER, msg.messageType, msg.messageContent, player,
                            human, sGlobals);
        } else {
            log.warn(
