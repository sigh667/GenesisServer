...
import com.genesis.gameserver.player.exception.PlayerInitException;
...
public class PlayerActor extends UntypedActor {
    ...
    public void onReceive(Object msg) throws Exception {
        try {
            ...
            if (msg instanceof HumanData) {
                ...
                this.dataUpdater = new DataUpdater("Player: " + humanData.humanId, 25,
                        TimeUnit.MINUTES.toMillis(5), this.sGlobals);
                ...
                try {
                    ...
                    this.dataUpdater = new DataUpdater("Player: " + humanInfo.getId(), 25,
                            TimeUnit.MINUTES.toMillis(5), this.sGlobals);
                    ...
                    if (humanInfo != null) {
                        String error = String.format(
                                "Human创建失败 Channel==%s,AccountId==%s,uuid==%d,name==%s",
                                humanInfo.getChannel(), humanInfo.getAccountId(), humanInfo.getId(),
                                humanInfo.getName());
                        log.error(error);
                    }
                    throw e;
            ...
            this.getContext()
                    .become(new PlayerActorNormalFunc(this, human, player, sGlobals, dataUpdater));
    ...
    private void initHuman(Human human) {
        ...
        if (!agentOption.isPresent()) {
            ...
            throw new PlayerInitException("Can not find AgentServer [" +
                    sGlobals.getServerConfig().getConnectedAgentServerId() + "] when player [" +
                    human.getAccountId() + "] is logining!");
        }
