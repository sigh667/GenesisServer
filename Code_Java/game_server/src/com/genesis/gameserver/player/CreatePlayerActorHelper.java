...
public class CreatePlayerActorHelper {
     ...
     */
    public static void createPlayerActor(Player player, HumanData humanData, ServerGlobals sGlobals,
            PlayerManagerArgs playerManagerArgs) {
        player.setUuid(humanData.humanId);
        ActorRef ar = playerManagerArgs.context
                .actorOf(Props.create(PlayerActor.class, player, sGlobals),
                        createPlayerActorName(player.getChannel(), player.getAccountId()));
        ar.tell(humanData, ActorRef.noSender());
        playerManagerArgs.context.watch(ar);
        playerManagerArgs.playerActorRefs
                .put(ar, new RegisterPlayer(player.getChannel(), player.getAccountId()));
     ...
     * @param playerManagerArgs
     */
    public static void createPlayerActor(Player player, HumanInfo humanInfo, ServerGlobals sGlobals,
            PlayerManagerArgs playerManagerArgs) {
