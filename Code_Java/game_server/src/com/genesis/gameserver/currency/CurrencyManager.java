package com.genesis.gameserver.currency;
...
public class CurrencyManager extends CurrencyContainer {
    ...
    public boolean giveMoney(CurrencyPropId currencyPropId, long addValue) {
        if (currencyPropId == null || addValue <= 0) {
            throw new IllegalArgumentException(
                    String.format("给钱参数有误：addValue=%d (addValue必须大于0), currency=%s (currency不能为空)",
                            addValue, currencyPropId));
        }

        if (currencyPropId == CurrencyPropId.CHARGE_DIAMOND ||
                currencyPropId == CurrencyPropId.FREE_DIAMOND) {
    ...
    private boolean giveDiamondMoney(CurrencyPropId currencyPropId, long addValue) {
        ...
        long allDiamond = currencyProp.get(CurrencyPropId.CHARGE_DIAMOND) +
                currencyProp.get(CurrencyPropId.FREE_DIAMOND);
    ...
    public void costMoney(CurrencyPropId currencyPropId, long costValue) {
        if (currencyPropId == null || currencyPropId == CurrencyPropId.CHARGE_DIAMOND ||
                currencyPropId == CurrencyPropId.FREE_DIAMOND || costValue <= 0) {
            throw new IllegalArgumentException(String.format(
                    "扣钱参数有误：costValue=%d (costValue必须大于0), currency=%s (currency不能为空，且不能是钻石)",
                    costValue, currencyPropId));
        }
        ...
        if (curMoney < costValue) {
            throw new IllegalArgumentException(
                    "金币不够！[Currency:" + currencyPropId + ", costValue:" + costValue +
                            ", curMoney:" + curMoney + "]");
        }
    ...
    public void costDiamond(long costValue) {
        ...
        if (curChargeDiamond + curFreeDiamond < costValue) {
            throw new IllegalArgumentException(
                    "钻石不够！[ChargeDiamond:" + curChargeDiamond + ", FreeDiamond:" + curFreeDiamond +
                            ", costValue:" + costValue + "]");
        }
        ...
        if (chargeDiamondCost > 0) {
            ...
            long accumulatedConsumedChargeDiamond =
                    this.currencyProp.get(CurrencyPropId.ACCUMULATED_CONSUMED_CHARGE_DIAMOND);
            this.currencyProp.set(CurrencyPropId.ACCUMULATED_CONSUMED_CHARGE_DIAMOND,
                    accumulatedConsumedChargeDiamond + chargeDiamondCost);
        }
    ...
    public List<Long> getCurrencyValues() {
        ...
        for (Currency currency : Currency.values()) {
            if (currency.isDiamond()) {
                currencyList.add(this.get(CurrencyPropId.CHARGE_DIAMOND) +
                        this.get(CurrencyPropId.FREE_DIAMOND));
            } else {
