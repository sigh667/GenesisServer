...
public abstract class AbstractAutoExecuteTask<H> implements ITimeEvent<H> {
    ...
    protected AbstractAutoExecuteTask(LocalTime autoExecuteTime, TimeAxis<H> timeAxis) {
        this.autoExecuteTimeList = Lists.newArrayList(autoExecuteTime);
        this.timeAxis = timeAxis;
    }

    protected AbstractAutoExecuteTask(List<LocalTime> autoExecuteTimeList, TimeAxis<H> timeAxis) {
    ...
    private long getNextAutoExecuteTime() {
        ...
        for (LocalTime autoExecuteTime : autoExecuteTimeList) {
            long eventTimeOfToday =
                    new DateTime(todayNow).withMillisOfDay(autoExecuteTime.getMillisOfDay())
                            .getMillis();
        ...
        // 明天的第一个自动刷新时间刷新货物
        return new DateTime(todayNow).withMillisOfDay(autoExecuteTimeList.get(0).getMillisOfDay())
                .getMillis() + TimeUtils.DAY;
    }
    ...
    protected final boolean isNeedExecute() {
        ...
        for (LocalTime autoExecuteTime : autoExecuteTimeList) {
            if (TimeUtils.canDoDailyResetOp(now, this.getLastExecuteTime(), autoExecuteTime)) {
                return true;
            }
        }
    ...
    public final int getSecondsBeforeNextAutoExecuteTime() {
        return (int) ((this.getNextAutoExecuteTime() - Globals.getTimeService().now()) / 1000);
    }
