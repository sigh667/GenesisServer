...
import com.icewind.protobuf.DailyRewardMessage.GCDailyRewardInfo;
...
public class DailyRewardManager {
    ...
    private final LocalTime resetTime = DailyTaskType.DAILY_REWARD_RESET_TIME.getSingleTime();
    //奖励英雄的位置，从0开始
    ...
    public void init() {
        ...
        //向时间轴注册每日奖励的刷新时间
        Globals.getTimeEventService()
                .startPeriodEventInRecentEventTime(human.getTimeAxis(), now, resetTime,
                        TimeUtils.DAY, new ITimeEvent<Human>() {
                            @Override
    ...
    private void resetTimesOfRewardAndIndexOfHeroIfInNewMonth() {
        if (TimeUtils
                .canDoMonthlyResetOp(Globals.getTimeService().now(), lastRewardTime, resetTime)) {
            timesOfReward = 0;
            this.indexOfHero = 6;
        }
    }
    ...
    private void initHeroRewardIndex(long now) {
        //当前时间和开服时间不是同一个月
        if (TimeUtils
                .canDoMonthlyResetOp(human.getServerGlobals().getServerStatus().getServerOpenTime(),
                        now, resetTime)) {
            return;
        }
    ...
    private Days daysToTheEndOfMonth(LocalDateTime current) {
        LocalDateTime amendedEndDayOfMonth =
                current.dayOfMonth().withMinimumValue().withMillisOfDay(resetTime.getMillisOfDay());
    ...
    public void giveHeroWhenNewCreateIfNecessary() {
        if (TimeUtils.isSameMonth(human.getServerGlobals().getServerStatus().getServerOpenTime(),
                human.getCreateTime().getTime())) {
            return;
        }
        LocalDateTime serverOpenDateTime =
                new LocalDateTime(human.getServerGlobals().getServerStatus().getServerOpenTime());
        Days daysToTheEndOfMonthSinceServerOpen = daysToTheEndOfMonth(serverOpenDateTime);
        if (daysToTheEndOfMonthSinceServerOpen.getDays() > 1) {
            return; //注释中第一个条件
        }

        LocalDateTime resetTimeInServerOpenDay =
                serverOpenDateTime.withMillisOfDay(resetTime.getMillisOfDay());
        if (serverOpenDateTime.isBefore(resetTimeInServerOpenDay)) {
            resetTimeInServerOpenDay.minusDays(1);
        }
        if (Days.daysBetween(resetTimeInServerOpenDay,
                new LocalDateTime(human.getCreateTime().getTime())).getDays() > 2) {
            return; //结合isSameMonth构成注释中第二个条件
        }
        //TODO:发邮件给英雄
    }
