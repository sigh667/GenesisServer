...
public class EnergyManager {
    ...
    public void init() {
        //离线时长 和 体力恢复周期
        long recoverInterval =
                Globals.getTimeService().now() - human.get(HumanPropId.LAST_ENERGY_RECOVER_TIME);
    ...
    public boolean isEnergyEnough(final int value) {
        if (value < 0) {
            throw new IllegalArgumentException(
                    String.format("玩家ID：d%，参数异常：value=%d (value必须>=0)", human.getDbId(), value));
        }
    ...
    public int costEnergy(final int value) {
        ...
        if (value == 0 || !isEnergyEnough(value)) {
            return 0;
        }
    ...
    public int addEnergy(final int addValue) {
        if (addValue <= 0) {
            throw new IllegalArgumentException(
                    String.format("玩家ID：d%，参数异常：value=%d (value必须>0)", human.getDbId(), addValue));
        }
        if (!this.isAddable()) {
            return 0;
        }
    ...
    public int recoverEnergy(final int recoverValue) {
        if (recoverValue <= 0) {
            throw new IllegalArgumentException(
                    String.format("玩家ID：d%，参数异常：value=%d (value必须>0)", human.getDbId(),
                            recoverValue));
        }
