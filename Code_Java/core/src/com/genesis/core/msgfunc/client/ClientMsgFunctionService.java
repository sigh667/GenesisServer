...
public class ClientMsgFunctionService {
    ...
    private ImmutableTable<MessageTarget, Integer, IClientMsgFunc<GeneratedMessage, MsgArgs, MsgArgs>>
            funcTable;
    private ImmutableMap<Integer, Parser<? extends GeneratedMessage>> paserMap;

    public ClientMsgFunctionService(
            Table<MessageTarget, Integer, IClientMsgFunc<GeneratedMessage, MsgArgs, MsgArgs>> funcTable,
            Map<Integer, Parser<? extends GeneratedMessage>> paserMap) {
        this.funcTable = ImmutableTable.copyOf(checkNotNull(funcTable,
                "MsgFunctionService can not init with null parsers!"));
        this.paserMap = ImmutableMap.copyOf(checkNotNull(paserMap,
                "MsgFunctionService can not init with null parsers!"));
    }

    public <Args1 extends MsgArgs, Args2 extends MsgArgs> void handle(MessageTarget messageTarget,
                                                                      int msgType, byte[] content, ISession session, Args1 arg1, Args2 arg2) {
        IClientMsgFunc<GeneratedMessage, MsgArgs, MsgArgs> msgFunc =
