package com.genesis.core.msgfunc.protobufutil;
...
public class MsgUtil {
    ...
    public static Parser extractMsgParser(Class<?> msgTypeClass)
        ...
        if (parser == null) {
            throw new RuntimeException("Class[" + msgTypeClass + "] 的解析器为null！可能消息类不正确");
        }
        return parser;
    }
    ...
    public static int getMsgType(Class<?> msgClass, boolean isCG) {
        try {
            ...
            if (isCG) {
                return d.getOptions().getExtension(MessageType.cgMessageType).getNumber();
            } else {
                return d.getOptions().getExtension(MessageType.gcMessageType).getNumber();
            }
        } catch (Exception e) {
