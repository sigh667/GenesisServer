...
import com.genesis.core.redis.op.actiononconnectfail.ConnectionFailRetryStrategy;
 ...
 *
 */
public class PipelineOpImpl implements IPipelineOp,
        IRealExecuteRedisCommand<PipelineProcess, IRedisResponse<List<IRedisResponse<?>>>> {
    ...
    public PipelineOpImpl(JedisPool jedisPool, ISerializerPool serializerPool,
        ...
        try {
            ...
            for (Object each : results) {
                ...
                if (!(each instanceof JedisDataException)) {
                    allResponse.add(RedisResponse
                            .success(cmd.processResult(each, client, serializer)));
                } else {
                    allResponse.add(RedisResponse
                            .fail(((JedisDataException) each).fillInStackTrace().toString()));
                }
            }
            ...
            if (serializer != null) {
                serializerPool.returnResource(serializer);
            }
        }
    }
