 ...
 *
 */
public class WatchedTransactionOpImpl extends AbstractTransactionOp
        implements IWatchedTransactionOp,
        IRealExecuteRedisCommand<TransactionProcess, IRedisResponse<List<IRedisResponse<?>>>> {
    ...
    WatchedTransactionOpImpl(JedisPool jedisPool, ISerializerPool serializerPool,
    ...
    public IRedisResponse<List<IRedisResponse<?>>> exec(TransactionProcess func) {
        if (!isInTransaction) {
            throw new RedisException(
                    "Can not execute redis transaction cmd in none transaction mode!");
        }
        try {
    ...
    public IRedisResponse<List<IRedisResponse<?>>> realExec(TransactionProcess func) {
        try {
            ...
            if (serializer != null) {
                this.serializerPool.returnResource(serializer);
            }
        }
    }
