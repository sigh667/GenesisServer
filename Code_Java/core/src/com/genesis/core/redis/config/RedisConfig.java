package com.genesis.core.redis.config;
...
public class RedisConfig {
    ...
    public RedisConfig(String serverKey, String host, int port, int initialSerializerObjectCount,
            int timeout) {
        this.host = host;
        this.port = port;
        this.serverKey = serverKey;
    ...
    public static HashMap<String, RedisConfig> getRedisConfigs() {
        Collection<Entry<String, ConfigValue>> redisList =
                config.getConfig("redis").root().entrySet();
        ...
        for (Entry<String, ConfigValue> each : redisList) {
            ...
            int initSerializerObjectCount =
                    initSerObjectCount != null ? (int) initSerObjectCount.unwrapped() : 1;
            redisMap.put(each.getKey(),
                    new RedisConfig(each.getKey(), host, port, initSerializerObjectCount));
        }
        return redisMap;
    }
