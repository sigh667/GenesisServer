package com.genesis.core.function;
    ...
    public static void assertIsFunctionClass(Class<?> clazz) {
        ...
        for (Field each : allField) {
            if (Modifier.isFinal(each.getModifiers())) {
                if (each.getType().isPrimitive() || each.getType() == String.class) {
                    continue;
                }
            }

            Class<?> type = each.getType();
            if (!type.isAssignableFrom(Logger.class) &&
                    !type.isAssignableFrom(GeneratedMessage.class)) {
                throw new NotAFunctionException(
                        clazz.getName() + " is not a Function class, " + each.getName() +
