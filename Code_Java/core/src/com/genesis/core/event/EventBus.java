package com.genesis.core.event;
...
public class EventBus {
    ...
    private Multimap<Class<?>, IEventListener<?>> listenerMap = ArrayListMultimap.create();

    public EventBus(List<String> packages) {
        if (packages == null || packages.size() == 0) {
            throw new RuntimeException("pakages argument cannot be null or empty!");
        }
        ...
        for (String packageStr : packages) {
            listenerClassSet
                    .addAll(PackageUtil.getPackageClasses(packageStr, new Predicate<Class<?>>() {
                        @Override
        ...
        try {
            ...
            for (Class<?> listenerClass : listenerClassSet) {
                ...
                // 从监听器类的泛型中获取事件类
                Class<?> eventClass =
                        GenericityUtil.extractFirstGenericType(listenerClass, IEventListener.class);
                listenerMap.put(eventClass, (IEventListener<?>) listenerClass.newInstance());
            }
        ...
        try {
            for (Object o : listenerMap.get(event.getClass())) {
                try {
                    ...
                    logger.error(String.format("event handle error, Event[%s] Listener[%s]",
                            event.getClass().getName(), o.getClass().getName()), e);
                }
            }
        } catch (Exception e) {
