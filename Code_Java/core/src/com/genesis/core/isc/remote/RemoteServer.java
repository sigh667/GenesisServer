...
public class RemoteServer implements IRemote {
    ...
    public RemoteServer(IActorPackages remoteActor, ServerConfig localConfig) {
        this.remoteActor =
                checkNotNull(remoteActor, "RemoteServer can not reference a null remote actor!");
        this.localConfig =
                checkNotNull(localConfig, "RemoteServer can not init with a null local config!");
