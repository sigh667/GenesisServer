...
import com.genesis.core.isc.remote.RemoteServer;
...
import com.genesis.core.isc.msg.Register;
...
public class ISCService {
    ...
    public ISCService(RemoteActorManager remoteActorManager, ServerConfig localConfig) {
        this.remoteActorManager = checkNotNull(remoteActorManager,
                "ISCService can not init with a null remote actor manager!");
        this.localConfig =
                checkNotNull(localConfig, "ISCService can not init with a null local config!");
     ...
     * @return
     */
    public boolean registerToRemote(ServerType sType, int sId, IActorPackages localActorPackages,
            MessageTarget... target) {
        checkNotNull(sType, "ISCService can not registerToRemote with null server type!");

        if (remoteTable.contains(sType, sId)) {
            return true;
        }
        ...
        Register register =
                new Register(localConfig.serverType, localConfig.serverId, localActorPackages);
