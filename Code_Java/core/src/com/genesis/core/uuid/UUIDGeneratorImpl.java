...
class UUIDGeneratorImpl implements IUUIDGenerator {
     ...
     * 			  UUID类型包数组
     */
    UUIDGeneratorImpl(int serverGroup, int serverId, Class<? extends IUUIDType> enumClass,
            IRedis iRedis) {
        if (enumClass == null) {
        ...
        for (IUUIDType uuidType : enumClass.getEnumConstants()) {
            ...
            final UUID64Template tmpl =
                    new UUID64Template(SERVERGROUP_BIT_NUM, SERVER_BIT_NUM, OBJECT_ID_BIT_NUM,
