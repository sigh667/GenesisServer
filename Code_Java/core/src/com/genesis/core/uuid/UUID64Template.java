package com.genesis.core.uuid;

import com.genesis.core.annotation.Immutable;
...
class UUID64Template {
     ...
     */
    UUID64Template(int serverGroupBitNum, int serverBitNum, int objectBitNum, int serverGroup,
            int serverId) {
        // 检查参数合法性
        ...
        if (serverGroup < 0 || serverId < 0) {
            throw new IllegalArgumentException("All the channelId, serverId must be >= 0");
        }
    ...
    public static void checkBitsMaxNum(int bitNum, long value) {
        ...
        if (value > maxValue || value < 0) {
            throw new IllegalArgumentException(
                    "Can't represent value[" + value + "] with " + bitNum + "bits!!");
        }
    }
