...
import com.genesis.core.encrypt.XorDecryptedInputStream;
...
public class TemplateService {
    ...
    public void init(URL cfgPath) {
        ...
        for (ExcelTemplateConfig cfg : templateXmlConfigs) {
            ...
            try (InputStream is = isXorLoad ? new XorDecryptedInputStream(xlsPath) :
                    new FileInputStream(xlsPath)) {
                this.getTemplateParser(cfg).parseXlsFile(cfg.getClasses(), templateTable, is, cfg);
            } catch (Exception e) {
                throw new ConfigException(
                        "Errors occurs while parsing xls file:" + cfg.getExcelFileName(), e);
            }
        }
        patchUpAndCheck();
        afterTemplateReady();
    ...
    private void afterTemplateReady() {
        Set<Class<?>> temp =
                PackageUtil.getSubClass("com.mokylin.bleach", IAfterTemplateReady.class);
        try {
            for (Class<?> each : temp) {
    ...
    private void loadTemplateXmlConfig(URL cfgPath) {
        ...
        for (Element eachExcelFile : excelFileElements) {
            ...
            if (StringUtils.isEmpty(excelFileName)) {
                throw new ConfigException("The file name in template.xml can not be empty.");
            }
            ...
            for (int i = 0; i < sheetElements.size(); i++) {
                ...
                if (StringUtils.isEmpty(className)) {
                    throw new ConfigException(
                            "The class for a sheet in template.xml can not be empty. File name: " +
                                    excelFileName);
                }
            ...
            ExcelTemplateConfig templateConfig =
                    new ExcelTemplateConfig(excelFileName, fileSheetClasses, configSheet);
    ...
    public boolean reload(String excelFileName, int sheetIndex, boolean needCheck) {
        if (templateXmlConfigs == null) {
            return false;
        }

        for (ExcelTemplateConfig cfg : templateXmlConfigs) {
            if (!excelFileName.equals(cfg.getExcelFileName())) {
                continue;
            }
            ...
            if (cfg.getClasses().length == 0) {
                ...
                try {
                    ...
                    throw new ConfigException(
                            "Errors occurs while parsing xls file:" + excelFileName, e);
                } finally {
                    try {
                        if (is != null) {
                            is.close();
                ...
                tmplFileParser.parseXlsFile(xlsPath, sheetIndex, cfg.getClasses()[sheetIndex],
                        templateTable);
            }
