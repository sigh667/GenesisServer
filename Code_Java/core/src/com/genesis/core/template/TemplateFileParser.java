...
import com.genesis.core.template.annotation.FixUpByCellRange;
...
public class TemplateFileParser {
    ...
    public void parseXlsFile(Class<?>[] classes,
            Table<Class<?>, Integer, TemplateObject> templateTable, InputStream is,
            ExcelTemplateConfig cfg) throws Exception {
     ...
     */
    protected Map<Integer, TemplateObject> parseXlsSheet(Sheet sheet, Class<?> clazz)
        ...
        try {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (methods[i].isAnnotationPresent(FixUpByCellRange.class)) {
                    FixUpByCellRange fixupByCellRange =
                            methods[i].getAnnotation(FixUpByCellRange.class);
    ...
    public void parseXlsFile(String xlsPath, int index, Class<?> clazz,
            Table<Class<?>, Integer, TemplateObject> templateTable) {
        InputStream is = null;
        try {
            ...
            throw new ConfigException(
                    "Errors occurs while parsing xls file:" + xlsPath + ",sheet:" + index, e);
        }
    }
