...
import com.genesis.core.config.exception.ConfigException;
import com.genesis.core.template.annotation.BeanFieldNumber;
import com.genesis.core.template.poi.SegmentRow;
...
public class TemplateObjectAssembler {
     ...
     */
    public static boolean invokeSetMethod(Class<?> clazz, Field field, Object obj, Object value)
    ...
    private static Method searchSetterMethod(Method[] methods, String name,
        ...
        for (int i = 0; i < methods.length; i++) {
            m = methods[i];
            if (m.getName() == internedName && m.getParameterTypes().length == 1 &&
    ...
    private Object assembleObject(Field field, Object obj, Row row) throws Exception {
        ...
        if (fieldType.isAnnotationPresent(ExcelRowBinding.class)) {
            ...
            throw new ConfigException("Can not assemble a object to a non row bingding object: " +
                    fieldType.getName());
        }
    }
    ...
    private Object getArray(Class<?> fieldType, ExcelCollectionMapping ecm, Row row)
            throws Exception {
    ...
    private void insertSetOrList(Collection<Object> col, ExcelCollectionMapping ecm, Row row)
            throws Exception {
            ...
            if (element_cl.isAnnotationPresent(ExcelRowBinding.class)) {
                ...
                if (strs.length > 1) {
                    throw new ConfigException(
                            String.format("cell's number greater than 1 on row(%d)",
                                    row.getRowNum()));
                }
    ...
    private void insertMap(Map<String, Object> map, ExcelCollectionMapping ecm, Row row)
            throws Exception {
        Class<?> element_cl = ecm.clazz();
        String cn = ecm.collectionNumber();
        String[] cns = cn.split(COLLECTION_SPLIT);
        ...
        try {
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(BeanFieldNumber.class)) {
                    ...
                    Object fValue =
                            getFieldValue(field, row, field.getType(), Integer.parseInt(cellIndex));
     ...
     * @throws Exception
     */
    private Object getFieldValue(Field field, Row row, Class<?> fieldType, int offset)
            throws Exception {
        Cell cell = row.getCell(offset);
     ...
     * @param fieldType
     */
    private void convertValueToType(String[] values, Object result, Class<?> fieldType) {
        ...
        if (fieldType == int.class || fieldType == Integer.class) {
            ...
            for (int i = 0; i < values.length; i++) {
                Array.setShort(result, i, (short) Double.parseDouble(values[i]));
            }
        } else if (fieldType == double.class || fieldType == Double.class) {
            for (int i = 0; i < values.length; i++) {
