package com.genesis.core.serviceinit;
...
public class ServiceInitManager {
     ...
     * @return
     */
    private static Set<Class<?>> getServiceClassSet(String packageName,
        ...
        return PackageUtil.getPackageClasses(packageName, new Predicate<Class<?>>() {
            ...
            public boolean apply(Class<?> input) {
                return ServiceInitializeRequired.class.isAssignableFrom(input) &&
                        input.isAnnotationPresent(anno);
            }
     ...
     */
    private static void recursiveCheckCircularDependency(Class<?> service,
            List<Class<?>> traversedClasses) {
    ...
    private static void realInitService(Class<?> service, List<Class<?>> serviceList) {
        ...
        if (serviceInit == null) {
            throw new WrongServiceObjectFieldException(
                    "Service class <" + service.getName() + "> don't have static <" +
                            service.getName() + "> field!");
        }
