...
public interface IValueOp extends ICommonOp{
	...
	/**
	 * 使用key将value放入redis，并且将该key设置为second秒后过期。仅当key不存在时，如果key存在则无效。<p>
	 * 
	 ...
	 * @return
	 */
	IRedisResponse<String> setIfAbsentExpiredBySeconds(String key, Object value, long second);
	
	/**
	 * 使用key将value放入redis，并且将该key设置为second秒后过期。仅当key存在时，如果key不存在则无效。<p>
	 * 
	 * 对应set key value ex seconds。<p>
	 * 
	 * 时间复杂度: O(1)
	 ...
	 * @return
	 */
	IRedisResponse<String> setIfPresentExpiredBySeconds(String key, Object value, long second);
	
	/**
	 * 使用key将value放入redis，并且将该key设置为millisenconds毫秒后过期。仅当key不存在时，如果key存在则无效。<p>
	 * 
	 ...
	 * @return
	 */
	IRedisResponse<String> setIfAbsentExpiredByMilliseconds(String key, Object value, long millisenconds);
	
	/**
	 * 使用key将value放入redis，并且将该key设置为millisenconds毫秒后过期。仅当key存在时，如果key不存在则无效。<p>
	 * 
	 * 对应set key value px millisenconds。<p>
	 * 
	 * 时间复杂度: O(1)
	 ...
	 * @return
	 */
	IRedisResponse<String> setIfPresentExpiredByMilliseconds(String key, Object value, long millisenconds);
