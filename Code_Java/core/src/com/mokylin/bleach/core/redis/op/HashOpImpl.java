...
public class HashOpImpl extends AbstractOp implements IHashOp {
    ...
    public HashOpImpl(JedisPool jedisPool, ISerializerPool serializerPool,
    ...
    public IRedisResponse<Long> hsetnx(final String key, final String field, final Object value) {
        return cmdOp.execCommand(new SingleRedisCmd<Long>() {
            @Override
            public void apply(Client jedis, ISerializer serializer) {
                jedis.hsetnx(SafeEncoder.encode(key), SafeEncoder.encode(field),
                        serializer.serialize(value));
            }
        });
    }
        ...
        return cmdOp.execCommand(new SingleRedisCmd<String>() {
            ...
            public void apply(Client jedis, ISerializer serializer) {
                if (maps == null || maps.size() == 0) {
                    throw new IllegalArgumentException("parameters of hmset can not be null!");
                }
                Map<byte[], byte[]> bMaps = new HashMap<>();
                for (Entry<String, ?> each : maps.entrySet()) {
                    bMaps.put(SafeEncoder.encode(each.getKey()),
                            serializer.serialize(each.getValue()));
                }
    ...
    @Override
    public IRedisResponse<Long> hdel(final String key, final String field,
            final String... moreFields) {
        return cmdOp.execCommand(new SingleRedisCmd<Long>() {
            ...
            public void apply(Client jedis, ISerializer serializer) {
                ...
                jedis.hdel(SafeEncoder.encode(key),
                        SafeEncoder.encodeMany(ArrayUtils.add(moreFields, field)));
            }
        });
    }
    ...
    public <T> IRedisResponse<Map<String, T>> hgetall(final String key, final Class<T> type) {
        return cmdOp.execCommand(new SingleRedisCmd<Map<String, T>>() {
            ...
            @Override
            public Map<String, T> processResult(Object value, Client jedis,
                    ISerializer serializer) {
                @SuppressWarnings("unchecked") List<byte[]> binaryValues = (List<byte[]>) value;
                ...
                while (iterator.hasNext()) {
                    valueMap.put(SafeEncoder.encode(iterator.next()),
                            serializer.deserialize(iterator.next(), type));
                }
                return valueMap;
            }
        });
