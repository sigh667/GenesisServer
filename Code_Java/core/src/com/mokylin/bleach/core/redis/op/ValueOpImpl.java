...
public class ValueOpImpl extends AbstractOp implements IValueOp {
    ...
    public ValueOpImpl(JedisPool jedisPool, ISerializerPool serializerPool,
    ...
    public IRedisResponse<String> setIfPresent(final String key, final Object value) {
        return cmdOp.execCommand(new SingleRedisCmd<String>() {
            ...
            public void apply(Client jedis, ISerializer serializer) {
                jedis.set(SafeEncoder.encode(key), serializer.serialize(value),
                        SafeEncoder.encode("XX"));
            }
        });
    }

    @Override
    public IRedisResponse<String> setExpiredBySeconds(final String key, final Object value,
            final long second) {
        return cmdOp.execCommand(new SingleRedisCmd<String>() {
            @Override
            public void apply(Client jedis, ISerializer serializer) {
                //                jedis.set(SafeEncoder.encode(key), serializer.serialize(value),
                //                        SafeEncoder.encode("EX"), second);
            }
        });
    }

    @Override
    public IRedisResponse<String> setExpiredByMilliseconds(final String key, final Object value,
            final long millisenconds) {
        return cmdOp.execCommand(new SingleRedisCmd<String>() {
            @Override
            public void apply(Client jedis, ISerializer serializer) {
                //                jedis.set(SafeEncoder.encode(key), serializer.serialize(value),
                //                        SafeEncoder.encode("PX"), millisenconds);
            }
        });
    }
