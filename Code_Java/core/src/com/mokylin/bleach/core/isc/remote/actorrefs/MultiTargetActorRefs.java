...
public class MultiTargetActorRefs implements IActorPackages, Serializable {
    ...
    public MultiTargetActorRefs(ServerType sType, int sId,
            Map<MessageTarget, Class<? extends UntypedActor>> targetClassMap,
            Map<Class<? extends UntypedActor>, ActorRef> targetActorRef) {
        checkNotNull(sType, "MultiTargetActorRefs can not init with null server type!");
        checkArgument(targetClassMap != null && targetClassMap.size() > 0,
                "MultiTargetActorRefs can not init with empty targetClassMap!");
        checkArgument(targetActorRef != null && targetActorRef.size() > 0,
                "MultiTargetActorRefs can not init with empty targetActorRef!");
        this.sType = sType;
        this.sId = sId;
        ...
        for (Entry<MessageTarget, Class<? extends UntypedActor>> each : targetClassMap.entrySet()) {
            ...
            if (actorRef == null) {
                throw new RuntimeException(
                        "MessageTarget [" + each.getKey() + "] map to Actor Class [" +
                                each.getValue().getName() +
                                "], but can not find corresponding ActorRef!");
            }
    ...
    public void sendMessage(Object sendingMsg, MessageTarget target) {
        ...
        if (actor != null) {
            ...
            log.warn("Can not find MessageTarget [{}] for Message [{}]", target.name(),
                    sendingMsg.getClass().getName());
        }
    }
